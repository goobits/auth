...
import { CSRF_COOKIE_NAME, CSRF_HEADER_NAME, issueCsrfToken, MemoryCsrfStore } from './security/csrf.js'
...
function resolveDefaults(config: AuthConfig): ResolvedDefaults {
	return {
		urlConfig: {
			allowedReturnToOrigins: config.urls?.allowedReturnToOrigins ?? [],
...
function createHandlers(
	...
	if (hasProviders) {
		loginHandler = createLoginHandler({
			allowedReturnToOrigins: urlConfig.allowedReturnToOrigins,
			providers,
