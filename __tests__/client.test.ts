...
describe('auth client', () => {
	...
	it('preserves the request contract for every non-WebAuthn operation', async () => {
		const fetcher = createQueuedFetcher([
			...
			},
			{
				success: true,
				secret: 'secret',
				otpauthUrl: 'otpauth://totp/bandamp',
				backupCodes: ['backup-1']
			},
		...
		expect(calls.map(([url, init]) => [String(url), init?.method])).toEqual([
			['https://bandamp.test/auth/magic-link', 'POST'],
			['https://bandamp.test/auth/magic-link/verify', 'POST'],
			['https://bandamp.test/auth/mfa/status', 'GET'],
			['https://bandamp.test/auth/mfa/enroll', 'POST'],
			['https://bandamp.test/auth/mfa/verify', 'POST'],
			['https://bandamp.test/auth/mfa/disable', 'POST'],
			['https://bandamp.test/auth/mfa/step-up', 'POST'],
			['https://bandamp.test/auth/mfa/backup-code', 'POST'],
			['https://bandamp.test/auth/sessions', 'GET'],
			['https://bandamp.test/auth/sessions', 'POST']
		])
	...
	it('honors endpoint overrides and validates OAuth provider input', async () => {
		...
		expect(vi.mocked(fetcher).mock.calls.map(([url]) => String(url))).toEqual([
			'https://bandamp.test/account/sessions',
			'https://bandamp.test/account/sessions/revoke'
		])
		expect(client.loginWithOAuth('google')).toBe('https://bandamp.test/auth/google')
	...
	it('converts WebAuthn options and credentials at the browser boundary', async () => {
		...
		await client.registerPasskey({ name: 'BandAmp key' })
		...
		expect(Array.from(requestOptions?.allowCredentials?.[0]?.id as Uint8Array)).toEqual([
			16, 17
		])
	...
	it.each([
		...
		},
		{
			...
			invoke: (client: ReturnType<typeof createAuthClient>) =>
				client.verifyMfa({ token: '123456' })
		},
		{
			name: 'session list',
