...
distribution, and runs `test:dist`. The distribution check treats `package.json`
as the public entrypoint inventory: workspace exports must target source while
published exports must target compiled output. It verifies the runtime-specific
password and WebAuthn builds, checks copied Svelte UI assets with `svelte-check`,
packs the package, imports every non-UI entrypoint under both Node and Worker
conditions, and rejects source or release-tool leakage from the artifact.
