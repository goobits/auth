...
`@goobits/auth/testing` exports test adapters and the canonical memory user adapter:
...
Use `security.rateLimit.windows` for custom policy. The former `max` and
`windowMs` shortcut is not accepted.
...
Configure `oauthTokenEncryption.encryptionKeyringJson` (or an application-owned
`tokenCodec`). During key rotation, keep the retired key in the keyring. Payloads
sealed by a retired key are lazily resealed with record-bound associated data
under the active key. Remove the retired key only after every retained token has
been read or an explicit bulk reseal has completed.
